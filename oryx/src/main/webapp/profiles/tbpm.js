...
}},handleMouseDown:function(b,a){if(this.active&&a instanceof ORYX.Core.Node){if(this.sourceNode){if(!this.createEdge(this.sourceNode,a)){return
}this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_LOADING_DISABLE})
}this.sourceNode=a
}else{if(this.active){if(this.connectButton){this.connectButton.toggle()
}}}},createEdge:function(c,e){var a=this.facade.getStencilSets().keys()[0];
...
this.facade.getCanvas().update();
this.edge=a
},rollback:function(){this.facade.deleteShape(this.edge);
this.facade.getCanvas().update();
...
},enableShapeTransform:function(){if(!this.shapeTransform){this.shapeTransform=true;
this.facade.registerOnEvent(ORYX.CONFIG.EVENT_PROPERTY_CHANGED,this.updateShapeSVG.bind(this))
}},updateShapeSVG:function(b){if(this.shapeTransform){var a=b.elements[0];
...
this.facade.getCanvas().update()
}},enableToggle:function(a,b){if(b&&$(this.canvasId).getStyle("backgroundImage")){$(this.canvasId).style.background="";
...
this.dialog.show()
},uploadImage:function(a,b){this.form.form.submit({url:this.tbpmImportServletURL,clientValidation:true,waitMsg:"Processing picture...",method:"POST",success:function(c,d){this.dialog.hide();
this.showConfirmDialog(d.response.responseText)
}.bind(this),failure:function(c,d){this.dialog.hide();
this.showConfirmDialog(d.response.responseText)
}.bind(this)})
},showConfirmDialog:function(a){var c=Ext.util.JSON.decode(a);
imgUri=c.uri;
...
}.bind(this)},{text:ORYX.I18N.TBPMSupport.imp.btnClose,handler:function(){b.close()
}.bind(this)}]});
b.show()
},processImport:function(b,d,e,a){var c=this.facade.getCanvas();
...
},addImageLayer:function(a){this.facade.getCanvas().properties["oryx-photo"]=a;
